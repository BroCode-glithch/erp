...
<body x-data="{ darkMode: false }" :class="{ 'dark': darkMode }" class="antialiased text-gray-800 bg-gray-100 dark:bg-gray-900 dark:text-white">
    ...
    <div class="flex flex-col items-center justify-center min-h-screen px-6 py-12 mt-8">
        ...
        <div class="max-w-4xl text-center animate-fade-in-up">
            ...
            <p class="text-lg text-gray-700 dark:text-gray-300">
                ...
                With real-time data, role-based access, and beautiful dashboards, {{ config('app.name') }} empowers your team with
        ...
        <div class="grid w-full max-w-6xl grid-cols-1 gap-6 mt-12 sm:grid-cols-2 lg:grid-cols-3">
            ...
            @foreach ($features as $index => $feature)
                <div
                    class="flex items-start p-6 space-x-4 transition-transform duration-300 transform bg-white rounded-lg shadow-md dark:bg-gray-800 feature-card hover:scale-105"
        ...
        <section class="w-full max-w-6xl mt-20">
            <h2 class="mb-6 text-2xl font-bold text-center text-indigo-600 dark:text-indigo-400">What Users Are Saying</h2>
            <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach ([
                    ['name' => 'Chinedu A.', 'comment' => 'This ERP made managing our students and departments so easy. Highly recommended!', 'role' => 'Registrar'],
                    ['name' => 'Fatima B.', 'comment' => 'Real-time dashboards helped us make better decisions. Game changer!', 'role' => 'Admin Officer'],
        ...
        <section class="w-full max-w-4xl mt-20">
            <h2 class="mb-6 text-2xl font-bold text-center text-indigo-600 dark:text-indigo-400">Frequently Asked Questions</h2>
            <div x-data="{ open: null }" class="space-y-4">
                @foreach ([
                    ['q' => 'Is the ERP mobile responsive?', 'a' => 'Yes, the UI adapts beautifully to mobile, tablet, and desktop devices.'],
                    ['q' => 'How secure is user data?', 'a' => 'We use Laravel best practices with encryption, roles, and location-aware login monitoring.'],
                    ['q' => 'Can it be integrated with other tools?', 'a' => 'Yes! Our system is built on Laravel, making API integration simple.'],
                ] as $i => $faq)
                    <div class="border rounded-lg dark:border-gray-600">
                        <button @click="open === {{ $i }} ? open = null : open = {{ $i }}"
                            ...
                            <svg x-show="open !== {{ $i }}" class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="open === {{ $i }}" class="px-4 pb-4 text-sm text-gray-600 dark:text-gray-300">
                            {{ $faq['a'] }}
                        </div>
                    </div>
                @endforeach
        ...
        <section class="w-full max-w-4xl mt-24 text-center">
            ...
            <a href="mailto:emmaariyom1@gmail.com" class="inline-block px-6 py-3 text-white bg-indigo-600 rounded-lg shadow hover:bg-indigo-700">
                Contact Us
            </a>
        </section>
        ...
        <!-- Footer -->
        <footer class="w-full mt-24 border-t dark:border-gray-700">
